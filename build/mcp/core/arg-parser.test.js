...
export async function extractArgPlaceholders(code) {
    const placeholders = [];
    let argIndex = 0;
    try {
        let ast;
            ...
            CallExpression(node) {
                ...
                if (node.callee.name === 'arg') {
                    ...
                    if (node.arguments.length >= 2 && node.arguments[1].type === 'ObjectExpression') {
                        ...
                        const placeholderProp = optionsObj.properties.find((prop) => prop.key.name === 'placeholder' || prop.key.value === 'placeholder');
