...
let packageNames = await arg("Which npm package/s would you like to install?", async (input) => {
    if (input.length < 3)
        return [];
    let response = await get(`http://registry.npmjs.com/-/v1/search?text=${input}&size=20`);
