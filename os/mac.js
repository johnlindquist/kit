...
let terminalEditor = editor => async (file) => {
    ...
    let possibleTerminals = () => Object.entries(supportedTerminalMap)
...
global.selectKitEditor = async (reset) => {
    let possibleEditors = () => [
        ...
        .filter(editor => exec(`PATH="/usr/bin:/usr/local/bin" which ${editor}`, { silent: true }).stdout)
