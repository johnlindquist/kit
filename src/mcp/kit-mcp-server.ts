...
// Import the parsing functions from test files
// In production, these would be moved to non-test files
import { extractArgPlaceholders } from "../core/arg-parser.test.js"
import { runScriptWithResult } from "../core/script-runner.test.js"
import { parseScriptFile, type DiscoveredScript } from "./script-discovery.test.js"

async function discoverAllScripts(directory: string): Promise<DiscoveredScript[]> {
...
function createToolSchema(script: DiscoveredScript): z.ZodObject<any> {
...
async function main() {
  ...
  for (const script of scripts) {
    ...
    server.tool(
      script.name.replace(/\s+/g, '-').toLowerCase(),
      script.metadata?.description || `Run the ${script.name} Script Kit script`,
      schema.shape,
      async (params: any) => {
