...
test('formatDistanceToNow - seconds', t => {
  ...
  t.is(formatDistanceToNow(date), 'less than 5 seconds')
  
  const date2 = createDateWithOffset({ seconds: 30 })
  t.is(formatDistanceToNow(date2), '30 seconds')
})
...
test('formatDistanceToNow - hours', t => {
  ...
  t.is(formatDistanceToNow(date), '1 hour')
  
  const date2 = createDateWithOffset({ hours: 5 })
  t.is(formatDistanceToNow(date2), '5 hours')
})
...
test('formatDistanceToNow - months', t => {
  ...
  t.is(formatDistanceToNow(date), '1 month')
...
test('formatDistanceToNow - years', t => {
  ...
  t.is(formatDistanceToNow(date), '1 year')
  
  const date2 = createDateWithOffset({ years: 5 })
  t.is(formatDistanceToNow(date2), '5 years')
})
...
test('format - basic date formatting', t => {
  ...
  t.is(format(date, 'hh:mm aaa'), '02:30 p.m.')
  t.is(format(date, "MMMM do, yyyy 'at' HH:mm".replace('do', 'eo')), 'January 15th, 2021 at 14:30')
})
...
test('formatDistanceToNowStrict - seconds', t => {
  ...
  t.is(formatDistanceToNowStrict(date), '30s')
  t.is(formatDistanceToNowStrict(date, { addSuffix: true }), '30s ago')
})

test('formatDistanceToNowStrict - minutes', t => {
  const date = createDateWithOffset({ minutes: 5 })
  t.is(formatDistanceToNowStrict(date), '5m')
  t.is(formatDistanceToNowStrict(date, { addSuffix: true }), '5m ago')
})

test('formatDistanceToNowStrict - hours', t => {
  const date = createDateWithOffset({ hours: 3 })
  t.is(formatDistanceToNowStrict(date), '3h')
  t.is(formatDistanceToNowStrict(date, { addSuffix: true }), '3h ago')
})

test('formatDistanceToNowStrict - days', t => {
  const date = createDateWithOffset({ days: 7 })
  t.is(formatDistanceToNowStrict(date), '7d')
  t.is(formatDistanceToNowStrict(date, { addSuffix: true }), '7d ago')
})

test('formatDistanceToNowStrict - months', t => {
  const date = createDateWithOffset({ months: 3 })
  t.is(formatDistanceToNowStrict(date), '3mo')
  t.is(formatDistanceToNowStrict(date, { addSuffix: true }), '3mo ago')
})

test('formatDistanceToNowStrict - years', t => {
  const date = createDateWithOffset({ years: 2 })
  t.is(formatDistanceToNowStrict(date), '2y')
  t.is(formatDistanceToNowStrict(date, { addSuffix: true }), '2y ago')
})
