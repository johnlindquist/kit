...
import { Channel, Env } from '../core/enum.js'
...
Please go to [${sponsorUrl}](${sponsorUrl}) to become a sponsor to unlock all features.
    ...
    case 'logout': {
      await setUserJson({})
      await global.cli('set-env-var', 'GITHUB_SCRIPTKIT_TOKEN', Env.REMOVE)
      await mainScript()
      break
    }
    default:
      global.warn(`Unexpected option: ${option}`)
  }
} else {
