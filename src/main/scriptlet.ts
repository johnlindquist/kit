...
export let runScriptlet = async (
  ...
  if (!scriptlet) {
    return await div(
      md(`# No template found for ${
        focusedScriptlet.value.name
      }
  ~~~json      
  ${JSON.stringify(focusedScriptlet, null, 2)}
  ~~~
...
await setSelectedText(result)`
    ...
    default: {
      ...
      if (
        getExtension(focusedScriptlet.tool) !==
        focusedScriptlet.tool
      ) {
        if (focusedScriptlet.tool) {
        }
      ...
      throw new Error(
        `Unsupported tool: ${focusedScriptlet.tool}`
      )
    }
  }
}
