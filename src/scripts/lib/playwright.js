...
export let select = async (url, selector, xf, { headless, timeout } = { headless: true, timeout: 5000 }) => {
    if (!xf)
        xf = el => el.innerText;
...
export let attribute = async (url, selector, attribute, { headless, timeout } = { headless: true, timeout: 5000 }) => {
