...
import { Key, Mode, Channel, UI, Value, PROMPT } from "../core/enum.js"
import {
	assignPropsTo,
	getMainScriptPath,
	cmd,
	...
	let debouncedChoiceFocus = debounce(
		async (input: string, state: AppState = {}) => {
			...
			if (global?.__currentPromptConfig?.shortcuts) {
				const shortcuts = global?.__currentPromptConfig?.shortcuts?.filter(
					(shortcut) => {
...
let onMenuToggleDefault = async (input, state) => {
	if (state.flaggedValue) {
		let localChoice: Choice = (global.kitPrevChoices || []).find(
			(c: Choice) => c.id === state?.focused?.id
		)
...
let prepPrompt = async (config: PromptConfig) => {
	...
	let mode =
		!choicesAreAFunction || !choicesHasAnInputArg ? Mode.FILTER : Mode.GENERATE
...
let determineChoicesType = (choices) => {
	...
	if (typeof choices === "function") {
		if (choices.constructor.name === "AsyncFunction") return "async"
		return "function"
...
global.div = async (htmlOrConfig = "", containerClasses = "") => {
	...
	if (config.html.trim() === "") htmlOrConfig = md("⚠️ html string was empty")
	return await global.kitPrompt({
		enter: `Continue`,
		shortcuts: [escapeShortcut],
		...config,
...
global.docs = async (filePath: string, options = {}) => {
	let { placeholder, groupedChoices, sections, tokens } =
		await global.groupMarkdownFileIntoChoices(filePath, options)

	let config =
		typeof options === "function" ? await options(sections, tokens) : options
...
global.basePrompt = async (
	...
	if (!choices) {
		height =
			PROMPT.HEIGHT.HEADER + PROMPT.INPUT.HEIGHT.SM + PROMPT.HEIGHT.FOOTER
	}
	...
	for await (let packageName of adjustedPackageNames) {
		...
		try {
			response = await get<{ downloads: number }>(
				`https://api.npmjs.org/downloads/point/last-week/` + stripVersion
			)
		} catch (error) {}
...
let { createNpm, createKenvPackageMissingInstall } = await import(
	"../api/npm.js"
)
global.npm = createNpm(appInstall)
global.npmInstall = createNpm(appInstall, false)
global.installMissingPackage = createKenvPackageMissingInstall(
	appInstall,
	false
)
...
global.getEditorHistory = async () => {
	return (await global.getDataFromApp(Channel.GET_EDITOR_HISTORY))?.state
		?.editorHistory
}
...
let addKitLibs = async (): Promise<ExtraLib[]> => {
	...
	let shelljsContent = await readFile(
		kitPath("node_modules", "@types", "shelljs", "index.d.ts"),
		"utf8"
	)
	...
	let nodeNotifierContent = await readFile(
		kitPath("node_modules", "@types", "node-notifier", "index.d.ts"),
		"utf8"
	)
...
global.screenshot = async (displayId?: number, bounds?: Rectangle) => {
	let config: ScreenshotConfig = {}
	if (displayId) config.displayId = displayId
	if (bounds) config.bounds = bounds
	let screenShotTmpPath = await sendWait(Channel.SCREENSHOT, config, 5000)
