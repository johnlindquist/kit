...
export const createPathChoices = async (
	...
	const dirFiles = await readdir(startPath, { withFileTypes: true })
	...
	const files = onlyDirs
		? []
		: validDirents.filter((dirent) => !folderSet.has(dirent.name))
	...
	const mapDirents = (dirents: Dirent[]): Choice[] => {
		return dirents.map((dirent) => {
			...
			const description =
				type === "folder"
					? ""
					: `${filesize(size)} - Last modified ${formatDistanceToNow(mtime)} ago`
