...
exports.scrapeAttribute = exports.scrapeSelector = exports.focusTab = exports.getTabs = exports.getActiveTab = void 0;
let getActiveTab = async (browser = "Google Chrome") => {
    let result = await applescript(String.raw `tell application "${browser}" to return URL of active tab of front window`);
    return result;
};
exports.getActiveTab = getActiveTab;
let getTabs = async (browser = "Google Chrome") => {
...
let focusTab = async (url, browser = "Google Chrome") => {
...
let scrapeSelector = async (url, selector, xf, { headless, timeout } = { headless: true, timeout: 5000 }) => {
    ...
    let results = await Promise.all(selectorHandles.map(async (handle) => {
...
exports.scrapeSelector = scrapeSelector;
let scrapeAttribute = async (url, selector, attribute, { headless, timeout } = { headless: true, timeout: 5000 }) => {
    let { chromium } = await npm("playwright");
...
exports.scrapeAttribute = scrapeAttribute;
