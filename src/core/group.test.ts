...
ava(
  "groupChoices - recent choices with custom recent flag",
  t => {
    const choices: Choice[] = [
...
ava("groupChoices - pass group with previewPath", t => {
  ...
  t.is(result[result.length - 1].choices[0].name, "Pass1")
  t.is(
    result[result.length - 1].choices[0].previewPath,
    "path/to/preview"
  )
  t.is(
    result[result.length - 1].choices[0].preview,
    undefined
  )
})
...
ava("groupChoices - benchmark performance", t => {
  ...
  t.log(
    `Average time per groupChoices call with 1000 choices: ${averageTime.toFixed(
      3
    )} ms`
  )
  t.pass()
