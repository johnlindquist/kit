...
export let errorPrompt = async (error: Error) => {
	...
	if (process.env.KIT_CONTEXT === "app") {
		...
		let stackWithoutId =
			error?.stack?.replace(/\?[^:]*/g, "") || "No Error Stack"
		...
		if (secondLine?.match("at file://")) {
			if (isWin) {
				errorFile = path.normalize(
					secondLine.replace("at file:///", "").replace(/:\d+/g, "").trim()
				)
...
global.log = (...args) => {
	if (process?.send) {
		global.send(
			Channel.KIT_LOG,
			args
				.map((a) => (typeof a != "string" ? JSONSafe.stringify(a) : a))
				.join(" ")
		)
	} else {
		_consoleLog(...args)
...
global.createChoiceSearch = async (
	choices: Choice[],
	config: Partial<
		Omit<Options, "keys"> & ConfigOptions & { keys: string[] }
	> = {
		minimumScore: 0.3,
		maxIterations: 3,
		keys: ["name"]
...
export function buildScriptConfig(
	...
	let scriptsConfig =
		typeof message === "string" ? { placeholder: message } : message
...
export let getApps = async () => {
	let { choices } = await readJson(kitPath("db", "apps.json")).catch(
		(error) => ({
			choices: []
		})
	)
...
let groupScripts = (scripts) => {
	let excludeGroups =
		global?.env?.KIT_EXCLUDE_KENVS?.split(",").map((k) => k.trim()) || []
...
export let getPreviewPath = (s: Script): string => {
	if (s?.previewPath) {
		return path.normalize(
			s.previewPath.replace("~", home()).replace("$KIT", kitPath())
		)
	}
	return path.resolve(
	...
	if (s?.preview === "false") {
		...
		try {
			let content = await readFile(
				path.resolve(path.dirname(s.filePath), s?.preview as string),
				"utf-8"
			)
		...
		if (typeof selectedKenv === "string") {
			return kenvChoices.find(
				(c) =>
					...
					path.resolve(c.value.dirPath) === path.resolve(selectedKenv as string)
			).value
		}
	}
