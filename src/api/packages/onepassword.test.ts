...
const clearModuleCache = () => {
    ...
    const onepasswordPath = require.resolve("./onepassword.js")
...
test.serial("op() should retrieve secret from default vault", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://GitHub Token/password"').resolves({
...
test.serial("op() should handle vault and field specification", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://dev-vault/API Key/credential"').resolves({
...
test.serial("op() should handle authentication errors and fall back to manual input", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').rejects(new Error("not authenticated"))
    execStub.withArgs('op signin').rejects(new Error("signin failed"))
...
test.serial("op() should cache results for subsequent calls", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://API Token/password"').resolves({
...
test.serial("env() should detect and handle 1Password references", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://dev-vault/API Key/password"').resolves({
        stdout: "secret-value\n"
    })
...
test.serial("op() should handle missing 1Password CLI", async (t) => {
    ...
    // CLI not installed
    execStub.withArgs('op --version').rejects(new Error("command not found"))
...
test.serial("op() should handle custom field names beyond 'password'", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://infrastructure/Database/username"').resolves({
        stdout: "db_admin\n"
    })
...
test.serial("op() should not send cache request for session duration", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://Session Token/password"').resolves({
        stdout: "session-only\n"
    })
...
test.serial("env() should support cache duration options for 1Password references", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://prod/Database/password"').resolves({
        stdout: "db-password\n"
    })
...
test.serial("createEnvVarName should handle special characters correctly", async (t) => {
    ...
    execStub.withArgs('op --version').resolves({ stdout: "2.30.0" })
    execStub.withArgs('op account list').resolves({ stdout: "account info" })
    execStub.withArgs('op read "op://my-vault/API-Key.2023/secret_field"').resolves({
